...
> **Documento visual:** `DESIGN.md` (Product Design v2, pós-paridade)
...
`DESIGN.md` é a fonte de verdade de **direção visual, craft e experiência**. Em conflito visual, `DESIGN.md` vence; em conflito de escopo/política/veracidade, este `PRODUCT.md` vence.
...
2. fase ativa de implementação (escopo §7);
...
21. UI/landing: seguir `DESIGN.md`, taste-skill, emilkowalski/skills, Figma e Playwright. Não usar Higgsfield.
22. Não inventar prova social, métricas ou clientes.
23. Preferir rotas e pastas já existentes no repositório (`/operacao`, `/previa/...`) em vez de recriar `/admin` ou `/preview` sem motivo.
...
Trabalho de product design da landing (composição, mídia, motion) é autorizado pela fase ativa visual em `DESIGN.md`, desde que não invente escopo funcional fora do §7.
...
A fase ativa de produto é **MVP 0 — Validação comercial e operação manual assistida**.

Em paralelo, a fase ativa de superfície pública é **product design da landing** (`DESIGN.md` v2): identidade Atria, mídia própria, motion com Emil skills, Figma e Playwright. Isso não autoriza crawler em lote, IA de geração, pagamentos ou publicação automática.

### Já entregue no repositório (baseline)

- landing pública `/`;
- demonstração fictícia `/previa/clinica-aurora`;
- formulário de solicitação de prévia com persistência (quando credenciais existem);
- área interna `/operacao` com autenticação de operador e lista/detalhe de leads;
- páginas legais `/privacidade` e `/termos`;
- comparação Atual / Proposta na landing e na prévia demonstrativa.

### Ainda em aberto no MVP 0

- preview privado por token para clínicas reais (`/preview/[token]` ou equivalente);
- cadastro manual completo de clínicas no painel;
- upload/registro sistemático de screenshots Atual/Proposta por lead;
- critérios de DoD comercial (§31) com venda e publicação reais.
...
Rota pública de exemplo: `/previa/clinica-aurora`.

Aviso obrigatório (copy de UI sem em-dash):
...
- Clínica Aurora nunca controla navegação, logo, voz institucional ou identidade visual da Atria.
...
### Incluído e já presente (manter / endurecer)

- landing pública `/`;
- formulário de solicitação de prévia;
- persistência de leads (Supabase quando configurado; comportamento honesto sem credenciais);
- autenticação de operador em `/operacao`;
- lista e detalhe simples de leads;
- demonstração fictícia `/previa/clinica-aurora`;
- comparação Atual / Proposta (demonstrativa);
- páginas `/privacidade` e `/termos`;
- product design da landing conforme `DESIGN.md` (Figma, Playwright, taste-skill, Emil; sem Higgsfield).

### Incluído e ainda a completar no MVP 0

- cadastro manual de clínicas no painel;
- upload ou registro manual de screenshots por lead/preview;
- criação manual de preview para clínica real;
- URL privada de preview (token);
- atualização manual de status com histórico;
...
- crawler em lote / aquisição automatizada (fundação experimental no repo não autoriza produto);
- score automático como feature pública;
- extração automática de conteúdo em produção;
...
- pipeline Higgsfield ou qualquer gerador de vídeo como dependência de entrega.
...
Preview privado por token (`app/preview/[token]/` ou equivalente) permanece no backlog do MVP 0.
...
## 18.0 Tipos de preview na fase ativa

1. **Demonstração fictícia (pública):** `/previa/clinica-aurora`
   Serve conversão e educação. Sempre com aviso de ficção.
2. **Preview privado de clínica real (a completar):** URL com token
   Serve venda e revisão. Sempre `noindex` / `nofollow`.

## 18.1 URL privada (alvo MVP 0)

```text
/preview/[token]
```

(Alternativa aceitável se já houver padrão consolidado no código: manter um único esquema e documentar aqui.)
...
- sem autenticação obrigatória para o prospect.

## 18.3 Conteúdo

- aviso de demonstração ou de proposta não publicada;
- screenshot ou representação Atual;
...
- Não construir crawler de aquisição antes da validação comercial.
...
- Não usar Higgsfield no pipeline visual ou de mídia.
- Direção visual da landing segue `DESIGN.md` Product Design v2 (taste-skill, Emil, Figma, Playwright).
- Operação interna vive em `/operacao`, não em um `/admin` paralelo inventado.
